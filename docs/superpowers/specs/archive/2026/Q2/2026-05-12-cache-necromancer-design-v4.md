# cache-necromancer 설계 v4

작성일: 2026-05-12 (v4 개정)
상태: 설계 (구현 전)
이전 버전: v1 / v2 / v3 (`2026-05-12-cache-necromancer-design{,-v2,-v3}.md`)

**v4 개정 사유**: v3 codex 재재리뷰에서 spec 정합성 + 새 silent failure 위험 MAJOR 4건 + MINOR 2건 발견.
- MAJOR #1 fire() API 인자 불일치 (state dict vs tmux_target 문자열 혼용)
- MAJOR #2 is_pane_safe_to_inject reason 미반환 (pane 영구 vs 일시 구분 불가)
- MAJOR #3 state lock timeout = hook timeout (graceful exit 마진 부족)
- MAJOR #4 fire 성공 후 Stop hook 누락 시 세션 영구 정지 (watchdog 부재)
- MINOR #1 delete_state lock 명시 누락
- MINOR #2 DriftDetector 첫 사이클 문구와 pseudocode 미세 모순

v4는 이 6개 항목을 모두 정확히 명세.

**v3 개정 사유 (참고)**: v2 codex 재리뷰에서 silent failure 위험 MAJOR 3건 + MINOR 2건 발견. (1) daemon lockfile truncate 버그, (2) state lock에서 blocking flock + spec timeout 불일치, (3) fire 안전 검증과 send-keys 사이 race 미처리, (4) recursion guard 경계 표기 혼용, (5) DriftDetector timezone-naive wall clock 사용. v3는 이 5개 항목을 모두 정확히 명세.
...
| state lock deadline | 4초 (hook timeout 5초 - cleanup margin 1초, v4) |
| fire→Stop watchdog | 120초 (v4) |
| send-keys 연속 실패 한도 | 3회 (v4) |
...
def update_state(sid_hash: str, mutator, *, allow_create: bool = False):
    ...
    try:
        lock_f = acquire_state_lock_with_timeout(lock_path, timeout=STATE_LOCK_DEADLINE)
...
def delete_state(sid_hash: str):
    """SessionEnd / poller GC에서 호출. per-session lock 획득 후 삭제 (MINOR #1)."""
    path = STATE_DIR / f"{sid_hash}.json"
    lock_path = STATE_DIR / f"{sid_hash}.lock"
    try:
        lock_f = acquire_state_lock_with_timeout(lock_path, timeout=STATE_LOCK_DEADLINE)
    ...
    try:
        ...
        fcntl.flock(lock_f, fcntl.LOCK_UN)
        lock_f.close()
    # lock_path는 그대로 남겨둔다 (재생성 비용 < 동시성 안전).
```

명세:
- **non-blocking flock + 4초 deadline retry**. blocking `LOCK_EX`는 절대 사용 안 함.
- **타이밍 invariant**: `hook_command_timeout (5s) > state_lock_deadline (4s) + cleanup_margin (≥1s)`. Python startup + import + config load + retry loop + logging + cleanup 전체가 5초 안에 들어옴. 이 invariant를 깨면 silent state 유실 위험. (MAJOR #3)
...
## fire 후 Stop hook 누락 watchdog (v4 MAJOR #4)

### 시나리오
1. `auto` 모드에서 send-keys 성공 → `next_refresh_at = None`, `last_fire_at = now` 기록.
2. Claude Code가 fire로 인한 요청을 처리하지만 응답 도중 강제 종료/타임아웃/네트워크 오류 발생.
3. Stop hook이 발화하지 않음 → `next_refresh_at`이 영원히 `None`.
4. 폴링 루프는 해당 세션을 후보에서 제외 → **세션 영구 정지 (silent failure)**.

### 해법
폴링 루프에서 `next_refresh_at is None and last_fire_at != None` 상태인 세션에 watchdog 적용:

```python
# config:
fire_stop_watchdog_seconds = 120   # fire 후 이 시간 내에 Stop hook 안 오면 복구

def watchdog_check(s: State, now: datetime, config: Config):
    if s.next_refresh_at is not None or s.last_fire_at is None:
        return  # 정상 상태
    elapsed = (now - s.last_fire_at).total_seconds()
    if elapsed > config.fire_stop_watchdog_seconds:
        log_warn(
...
watchdog은 **`refresh_count`를 증가시키지 않음** (이미 한 번 fire했으므로). 복구된 세션은 다음 정상 주기에 합류.

### 한계
- watchdog이 복구한 직후 늦게 Stop hook이 도착하면 `is_injected_response`가 `last_fire_at == None`을 보고 정상 Stop으로 처리 → `next_refresh_at`이 중복 설정되지만 같은 값으로 덮어쓰는 거라 무해.
- fire는 성공했는데 캐시가 실제 갱신됐는지(=Claude가 처리한 응답이 있는지)는 spec 외부에서 알 수 없음. 그래서 watchdog은 "다음 주기 진입 보장"만 책임짐.
...
### `daemon/refresh.py:fire()` (v4 — structured reason + API 통일)
...
class FireReason(str, Enum):
    ...
    PANE_GONE = "pane_gone"                # 영구: state 삭제 정책
    PANE_PROCESS_DEAD = "pane_process_dead" # 영구: state 삭제 정책
    NOT_CLAUDE = "not_claude"               # 일시: 재시도
    BUFFER_DIRTY = "buffer_dirty"           # 일시: 재시도
    CAPTURE_FAILED = "capture_failed"       # 일시: 재시도
    PANE_DRIFT = "pane_drift"               # 일시: 재시도 (직전 vs 재확인 결과 불일치)
    SEND_KEYS_FAILED = "send_keys_failed"   # 일시 누적: 3회 연속 시 비활성화

PERMANENT_REASONS = {FireReason.PANE_GONE, FireReason.PANE_PROCESS_DEAD}
...
def query_pane_by_id(pane_id: str) -> Optional[PaneInfo]:
    ...
    for line in out.splitlines():
        ...
        if pid_field == pane_id:
            return PaneInfo(pid_field, pane_pid, cmd)
    return None

def is_pane_safe_to_inject(pane_id: str) -> Tuple[bool, Optional[PaneInfo], FireReason]:
    """5단계 검증. 호출자가 reason으로 정책 분기. (v4 MAJOR #2)"""
    info = query_pane_by_id(pane_id)
    if not info:
        log_warn(f"pane gone: {pane_id}")
        return False, None, FireReason.PANE_GONE

    # 1. pane PID 살아있는지
    try:
        os.kill(int(info.pane_pid), 0)
    except (ProcessLookupError, ValueError):
        log_warn(f"pane process dead: pid={info.pane_pid}")
        return False, info, FireReason.PANE_PROCESS_DEAD

    # 2. current command이 claude 계열인지
    if "claude" not in info.current_cmd.lower():
        log_warn(f"pane not running claude: cmd={info.current_cmd}")
        return False, info, FireReason.NOT_CLAUDE
    ...
    try:
        ...
        return False, info, FireReason.CAPTURE_FAILED
    last = tail.strip()
    if last and not last.endswith((">", "│", "❯", "$", "#")):
        log_warn(f"pane buffer not clean: {last!r}")
        return False, info, FireReason.BUFFER_DIRTY

    return True, info, FireReason.OK

def fire(state: dict, prompt: str = ".") -> Tuple[bool, FireReason]:
    """v4: 호출자는 항상 state dict (또는 dataclass)를 그대로 전달.
    Returns (success, reason). reason은 호출자가 정책 분기에 사용.
    """
    pane_id = state.get("pane_id")
    if not pane_id:
        return False, FireReason.NO_PANE_ID

    # 1차 검증
    ok, info, reason = is_pane_safe_to_inject(pane_id)
    if not ok:
        return False, reason

    # 2차 재확인 (send-keys 직전, race 최소화)
    ok2, info2, reason2 = is_pane_safe_to_inject(pane_id)
    if not ok2:
        return False, reason2
    if info2 != info:
        log_warn(f"pane changed between checks: {info} -> {info2}")
        return False, FireReason.PANE_DRIFT
    ...
    try:
        ...
        return False, FireReason.SEND_KEYS_FAILED

    log_info(f"fired: pane_id={pane_id}")
    return True, FireReason.OK
```

### 실패 처리 정책 (MAJOR #2)

호출자(`execute_mode`)가 `FireReason`에 따라 분기:

- `PANE_GONE`, `PANE_PROCESS_DEAD`: **영구 소멸 → `delete_state(sid_hash)`**.
- `NOT_CLAUDE`, `BUFFER_DIRTY`, `CAPTURE_FAILED`, `PANE_DRIFT`: 일시적 → log 경고만. 다음 폴링 사이클 재시도.
- `SEND_KEYS_FAILED`: state에 `send_keys_failures` 카운터 증가. 3회 연속 시 `tmux_target = None`, `pane_id = None`으로 비활성화 후 log.
...
def handle_fire_result(s: State, success: bool, reason: FireReason, config: Config):
    ...
    if reason in PERMANENT_REASONS or reason == FireReason.NO_PANE_ID:
        ...
        return
    if reason == FireReason.SEND_KEYS_FAILED:
        update_state(s.sid_hash, lambda x: {**x,
...
- 데몬 부팅 직후 첫 `mark_sleep_start()` 호출 전엔 `last_mono=None`이라 `detect_after_sleep()`이 0을 반환 (= 데몬 시작 이전 sleep gap은 측정 대상 아님). **첫 `mark_sleep_start()` 이후 sleep부터는 모든 후속 sleep을 정상 감지** (v4 MINOR #2 문구 정정).
...
def run_poll_loop(config: Config):
    ...
    while True:
        ...
        # 3. fire→Stop watchdog (v4 MAJOR #4)
        for s in sessions:
...
- `watchdog_check`은 fire 후 Stop hook 누락으로 영구 정지된 세션을 복구 (MAJOR #4).
- DriftDetector는 sleep 직후에만 평가. 데몬 부팅 후 첫 `mark_sleep_start()` 이전엔 평가 기준점이 없어 감지 skip — 첫 sleep 이후부터는 정상 감지 (MINOR #2 문구 정정).

## 모드별 실행 (v4 — fire() API 통일 + handle_fire_result 사용)
...
def execute_mode(s: State, mode: str, config: Config):
    if mode == "notify":
        ...
        notifier.notify(
            f"🔄 {s.session_id[:8]} {config.refresh.hybrid_wait_seconds}s 내 "
            f"입력 없으면 자동 갱신"
        )
...
fire_stop_watchdog_seconds = 120         # v4: fire 후 Stop hook 누락 복구
send_keys_max_consecutive_failures = 3   # v4: 누적 실패 시 비활성화
state_lock_deadline_seconds = 4.0        # v4: hook timeout 5s - margin 1s
```
...
| fire 성공 후 Stop hook 누락 | watchdog (`fire_stop_watchdog_seconds` 후 `next_refresh_at` 자동 복구) (v4 MAJOR #4) |
| send-keys 연속 실패 | `send_keys_failures` 카운터 3회 도달 시 세션 비활성화 (`tmux_target=None`, `pane_id=None`) (v4 MAJOR #2) |
| SessionEnd ↔ update_state race | `delete_state()`도 per-session lock 획득 후 삭제 (v4 MINOR #1) |
...
- fire→Stop watchdog (v4): auto fire 성공 → Stop hook 안 옴 → 120s 후 `next_refresh_at` 자동 복구 검증
- fire reason 분기 (v4): PANE_GONE → state 삭제 / NOT_CLAUDE → 재시도 / SEND_KEYS_FAILED 3회 → 비활성화
- delete_state lock 경합 (v4): SessionEnd와 update_state 동시 호출 → 일관된 결과
- state lock timeout (v4): 4초 deadline + 1초 cleanup margin 내에 hook이 완료되는지 측정
